...
- [ ] NSIS installer (per-user, no admin needed) — first `npm run dist` in progress
- [ ] Portable single exe — verify the data dir lands next to the exe
- [ ] Verify the updater banner appears (and self-update stays off) in portable
- [ ] Install the built exe and confirm setup runs from a clean machine state
